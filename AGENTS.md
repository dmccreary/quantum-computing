...
These skills target teachers on the **Codex Pro plan**, which has a limited token budget. Teachers are **not sensitive to run times** — a task that takes 3 minutes instead of 1 minute is fine, but a task that burns excess tokens means they can do fewer tasks.
